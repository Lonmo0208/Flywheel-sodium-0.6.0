 ...
 */
public class BatchedMaterialGroup<P extends WorldProgram> implements MaterialGroup {

	protected final BatchingEngine<P> owner;
	protected final RenderType type;

	private final Map<Instanced<? extends InstanceData>, BatchedMaterial<?>> materials = new HashMap<>();

	private ModelAllocator allocator;
	private int vertexCount;
	private int instanceCount;

	public BatchedMaterialGroup(BatchingEngine<P> owner, RenderType type) {
		this.owner = owner;
		this.type = type;
	}
	...
	protected void renderAll(Matrix4f viewProjection, double camX, double camY, double camZ, RenderLayer layer) {
		...
		for (Map.Entry<Instanced<? extends InstanceData>, BatchedMaterial<?>> entry : materials.entrySet()) {
			BatchedMaterial<?> material = entry.getValue();
			...
			for (CPUInstancer<?> instancer : material.getAllInstancers()) {
	...
	private void initializeInstancers() {
		...
		for (BatchedMaterial<?> material : materials.values()) {
			for (CPUInstancer<?> instancer : material.uninitialized) {
